...
func TestEditProduct_Success(t *testing.T) {
	...
	mock, editCalled := createEditProductMock(t, command)

	uc := NewEditProduct(mock)
	err := uc.Execute(command)

	assert.Nil(t, err)
	assert.Equal(t, 1, int(*editCalled))
}
...
func TestEditProductName_Fail(t *testing.T) {
	...
	assert.NotNil(t, err)
	assert.Contains(t, err.Error(), "product name is empty")
